...
export default function EmployeeSettings() {
  ...
  const handleSave = async () => {
    try {
      ...
      alert('Profile synced with Database successfully!');
    } catch (err) {
      alert('Sync failed.');
    }
  };

  if (loading) return <div className="p-12 text-center text-gray-500 font-bold animate-pulse">Synchronizing Identity...</div>;

  return (
    <div className="max-w-4xl mx-auto px-6 py-12">
      <div className="mb-10 border-b border-brand-border/50/50 pb-6">
         <h1 className="text-3xl font-black text-white tracking-tight flex items-center gap-3">
            <User className="text-brand-accent" size={32} /> My Profile
         </h1>
         <p className="text-gray-400 mt-2 font-medium">Manage your personal information, identity access, and profile photo.</p>
      </div>
      ...
      <div className="glass-card backdrop-blur-xl p-8 rounded-[2rem] border border-brand-border/50/50 shadow-2xl flex flex-col md:flex-row gap-12 items-start">
         ...
         <div className="flex-1 w-full space-y-6">
            <div className="bg-brand-bg/50 border border-brand-primary/20 rounded-2xl p-4 flex items-start gap-4">
              <ShieldCheck className="text-emerald-500 mt-1" size={24} />
              <div>
                <h4 className="text-white font-bold text-sm">Identity Verified (DB Synced)</h4>
                <p className="text-xs text-gray-400 mt-1 font-medium">Your account is secured under the IntelliCore infrastructure. Data is persisted in the central MongoDB.</p>
              </div>
            </div>
            ...
            <div>
               ...
               />
               <p className="text-[10px] text-gray-500 mt-2 ml-1">Contact IT to change your corporate email.</p>
            </div>
